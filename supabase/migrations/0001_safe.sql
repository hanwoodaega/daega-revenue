...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'manager_must_have_branch'
...
drop policy if exists "profiles updatable by admin" on profiles;
...
drop policy if exists "sales readable by admin or branch manager" on sales_entries;
...
drop policy if exists "sales insertable by admin or branch manager" on sales_entries;
...
begin
  if not exists (
    ...
    from pg_policies
    where schemaname = 'public'
      and tablename = 'sales_entries'
      and policyname = 'sales updatable by admin or branch manager'
  ) then
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'sales_entries'
      and policyname = 'sales deletable by admin or branch manager'
  ) then
